...
const keyPress = (input, button) => {
  document.getElementById(input).addEventListener("keypress", function (event) {
    if (event.key === 'Enter')
      document.getElementById(button).click();
  });
}
keyPress('search','search-btn');
keyPress('duration','create-slider');
...
const showImages = (images) => {
  imagesArea.style.display = 'd-none';
  gallery.innerHTML = '';
  // show gallery title
...
searchBtn.addEventListener('click', function () {
  ...
  document.getElementById('search-topic').innerText = search;
  if(!search){
    alert("Enter a name...");
  }
  else{
